...
const VotersList = db.votersLists
...
const getAllNominees = async (req, res)  => {
    let votersLists = await VotersList.findAll({
        
    })
    res.status(200).send(votersLists)
...
*/
// 3A. get Resident Nominees by position

const getAllNomineesbyPosition = async (req, res)  => {

    let vposition = req.params.position
    let votersLists = await VotersList.findAll({ where: { position: vposition} })
    res.status(200).send(votersLists)
